...
function generateIntArray(
  ...
  minValue,
  maxValue,
  { unique = false, sortedOrder = false } = {}
) {
  let array;

  if (unique) {
    const space = maxValue - minValue + 1;
    length = Math.min(length, space);

    if (space <= 1_000_000) {
      const range = Array.from({ length: space }, (_, i) => i + minValue);
      _shuffle(range);
      array = range.slice(0, length);
    } else {
      const seen = new Set();
      while (seen.size < length) {
        seen.add(generateInt(minValue, maxValue));
      }
      array = Array.from(seen);
    }
  } else {
...
function generateIntMatrix(
  ...
  return generate2dArray(
    rows,
    cols,
    () => generateInt(minValue, maxValue),
...
function _generateBst(nodes, minValue, maxValue, balanced) {
  ...
  const values = generateIntArray(nodes, minValue, maxValue, {
    unique: true,
    sortedOrder: true,
  });
